  ...
  <div class="app-container">
    <el-card class="mb-2">
      <el-form ref="formRef" :model="formModel">
        <el-form-item :label="t('Video.CartoonComic.InputFolder')" prop="input">
          <el-input v-model="formModel.input" :placeholder="t('Video.CartoonComic.InputFolder')">
            <template #append>
              <el-button @click="handleInput">
                {{ t("Video.CartoonComic.InputSelect") }}
              </el-button>
            </template>
          </el-input>
        </el-form-item>
        <el-form-item :label="t('Video.CartoonComic.OutputFolder')" prop="output">
          <el-input v-model="formModel.output" :placeholder="t('Video.CartoonComic.OutputFolder')">
            <template #append>
              <el-button @click="handleOutput">
                {{ t("Video.CartoonComic.OutputSelect") }}
              </el-button>
            </template>
          </el-input>
        </el-form-item>
        <el-space wrap :size="20">
          <el-form-item :label="t('Video.CartoonComic.Provider')" prop="provider">
          ...
          <el-form-item :label="t('Video.CartoonComic.Mode')" prop="mode">
          ...
          <el-form-item :label="t('Video.CartoonComic.Quality')" prop="quality">
...
import API from "@/api/video/cartoonComic.api";
...
const modeOptions = [
  { value: "hayao", label: t("Video.CartoonComic.ModeHayao") },
  { value: "cute", label: t("Video.CartoonComic.ModeCute") },
  { value: "jpface", label: t("Video.CartoonComic.ModeJPFace") },
  { value: "shinkai", label: t("Video.CartoonComic.ModeShinkai") },
  { value: "sketch", label: t("Video.CartoonComic.ModeSketch") },
];
const qualityOptions = [
  { value: "auto", label: t("Video.CartoonComic.QualityAuto") },
  { value: "high", label: t("Video.CartoonComic.QualityHigh") },
  { value: "medium", label: t("Video.CartoonComic.QualityMedium") },
  { value: "low", label: t("Video.CartoonComic.QualityLow") },
];
...
function handlefileGridTable(val: FileTable | undefined) {
  if (val) {
    API.getFileUrl(val.path).then((data) => {
      fileViewRef.value.videoRef.src = data.url;
    });
  }
}
...
onMounted(() => {
  ...
  fileMessageRef.value.addMessage({ type: "info", text: t("Video.CartoonComic.Help") });
});
