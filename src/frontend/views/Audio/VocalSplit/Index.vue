  ...
  <div class="app-container">
    <el-card class="mb-2">
      <el-form ref="formRef" :model="formModel">
        <el-form-item :label="t('Audio.VocalSplit.InputFolder')" prop="input">
          <el-input v-model="formModel.input" :placeholder="t('Audio.VocalSplit.InputFolder')">
            <template #append>
              <el-button @click="handleInput">
                {{ t("Audio.VocalSplit.InputSelect") }}
              </el-button>
            </template>
          </el-input>
        </el-form-item>
        <el-form-item :label="t('Audio.VocalSplit.OutputFolder')" prop="output">
          <el-input v-model="formModel.output" :placeholder="t('Audio.VocalSplit.OutputFolder')">
            <template #append>
              <el-button @click="handleOutput">
                {{ t("Audio.VocalSplit.OutputSelect") }}
              </el-button>
            </template>
          </el-input>
        </el-form-item>
        <el-space wrap :size="20">
          <el-form-item :label="t('Audio.VocalSplit.Provider')" prop="provider">
          ...
          <el-form-item :label="t('Audio.VocalSplit.Mode')" prop="mode">
    ...
    <el-row :gutter="10">
      ...
      <el-col :sm="12">
        <el-card class="mb-2">
          <FileView ref="fileViewRef" :audioVisible="true" />
        </el-card>
      </el-col>
      <el-col :sm="6">
        <el-card>
...
import API from "@/api/audio/vocalSplit.api";
...
const modeOptions = [{ value: "standard", label: t("Audio.VocalSplit.ModeStandard") }];
...
function handlefileGridTable(val: FileTable | undefined) {
  if (val) {
    API.getFileUrl(val.path).then((data) => {
      fileViewRef.value.audioRef.src = data.url;
    });
  }
}
...
onMounted(() => {
  ...
  fileMessageRef.value.addMessage({ type: "info", text: t("Audio.VocalSplit.Help") });
});
