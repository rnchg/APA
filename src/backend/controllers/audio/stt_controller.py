...
from core.services.audio.stt.index_service import IndexService as STTService
    ...
    def setup_routes(self):
        ...
        def audio_stt_open():
            ...
            if not os.path.exists(path):
                return self.badRequest(message=f"Audio.STT.OpenError: path: {path}")
            os.startfile(path)
            return self.ok()
    ...
    def start(self, data):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception("Audio.STT.InputError")
            output = data["output"]
            if not os.path.isdir(output):
                raise Exception("Audio.STT.OutputError")
            input_files = data["input_files"]
            if not input_files:
                raise Exception("Audio.STT.FileError")
            provider = data["provider"]
            if provider is None:
                raise Exception(f"Audio.STT.ParamError: provider: {provider}")
            mode = data["mode"]
            if mode is None:
                raise Exception(f"Audio.STT.ParamError: mode: {mode}")
            lang = data["lang"]
            if lang is None:
                raise Exception(f"Audio.STT.ParamError: lang: {lang}")
            similarity = data["similarity"]
            if similarity is None:
                raise Exception(f"Audio.STT.ParamError: similarity: {similarity}")
            yield from self.base_service.start(input, output, input_files, provider, mode, lang, similarity)
            self.base_service.is_stop = True
            self.base_service.message = {"type": "success", "text": "Audio.STT.ProcessEnd"}
        except ActivationException:
            self.base_service.is_auth = False
            self.base_service.message = {"type": "error", "text": "Audio.STT.ProcessAuthError"}
