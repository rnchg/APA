...
from core.services.audio.vocal_split.index_service import IndexService as VocalSplitService
...
class VocalSplitController(BaseController):
    ...
    def setup_routes(self):
        ...
        def audio_vocal_split_getFileUrl():
    ...
    def start(self, data):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception("Audio.VocalSplit.InputError")
            output = data["output"]
            if not os.path.isdir(output):
                raise Exception("Audio.VocalSplit.OutputError")
            input_files = data["input_files"]
            if not input_files:
                raise Exception("Audio.VocalSplit.FileError")
            provider = data["provider"]
            if provider is None:
                raise Exception(f"Audio.VocalSplit.ParamError: provider: {provider}")
            mode = data["mode"]
            if mode is None:
                raise Exception(f"Audio.VocalSplit.ParamError: mode: {mode}")
            yield from self.base_service.start(input, output, input_files, provider, mode)
            self.base_service.is_stop = True
            self.base_service.message = {"type": "success", "text": "Audio.VocalSplit.ProcessEnd"}
        except ActivationException:
            self.base_service.is_auth = False
            self.base_service.message = {"type": "error", "text": "Audio.VocalSplit.ProcessAuthError"}
