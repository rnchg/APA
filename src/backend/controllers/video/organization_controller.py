...
from core.services.video.organization.index_service import IndexService as OrganizationService
...
class OrganizationController(BaseController):
    def __init__(self):
        ...
        self.input_exts = [".videoinfo", ".json"]
        self.output_exts = AppConst.video_exts
        self.base_service = OrganizationService()
        self.base_url = f"{self.base_api}/video/organization"

    def setup_routes(self):

        @self.base_server.route(f"{self.base_url}/getConfig", methods=["POST"])
        def video_organization_getConfig():
            data = Current.config.video_organization.to_dict()
            return self.ok(data)

        @self.base_server.route(f"{self.base_url}/setConfig", methods=["POST"])
        def video_organization_setConfig():
            data = request.json["data"]
            Current.config.video_organization.dict_to(data)
            return self.ok()

        @self.base_server.route(f"{self.base_url}/getFolder", methods=["POST"])
        def video_organization_getFolder():
            path = request.json["path"]
            folders = webview.windows[0].create_file_dialog(webview.FOLDER_DIALOG, directory=path)
            if not folders:
                return self.badRequest(message=f"Video.Organization.FolderError: path: {path}")
            return self.ok({"folder": folders[0]})

        @self.base_server.route(f"{self.base_url}/getFileGrid", methods=["POST"])
        def video_organization_getFileGrid():
        ...
        def video_organization_getFileUrl():
    ...
    def start(self, data):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception("Video.Organization.InputError")
            output = data["output"]
            if not os.path.isdir(output):
                raise Exception("Video.Organization.OutputError")
            input_files = FileHelper.get_paths(input)
            if not input_files:
                raise Exception("Video.Organization.FileError")
            client = data["client"]
            if client is None:
                raise Exception(f"Video.Organization.ParamError: client: {client}")
            yield from self.base_service.start(input, output, input_files, client)
            self.base_service.is_stop = True
            self.base_service.message = {"type": "success", "text": "Video.Organization.ProcessEnd"}
        except ActivationException:
            self.base_service.is_auth = False
            self.base_service.message = {"type": "error", "text": "Video.Organization.ProcessAuthError"}
