...
from core.services.video.matting.index_service import IndexService as MattingService
...
class MattingController(BaseController):
    def __init__(self):
        ...
        self.base_service = MattingService()
        self.base_url = f"{self.base_api}/video/matting"

    def setup_routes(self):

        @self.base_server.route(f"{self.base_url}/getConfig", methods=["POST"])
        def video_matting_getConfig():
            data = Current.config.video_matting.to_dict()
            return self.ok(data)

        @self.base_server.route(f"{self.base_url}/setConfig", methods=["POST"])
        def video_matting_setConfig():
            data = request.json["data"]
            Current.config.video_matting.dict_to(data)
            return self.ok()

        @self.base_server.route(f"{self.base_url}/getFolder", methods=["POST"])
        def video_matting_getFolder():
    ...
    def start(self, data):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception("Video.Matting.InputError")
            output = data["output"]
            if not os.path.isdir(output):
                raise Exception("Video.Matting.OutputError")
            input_files = data["input_files"]
            if not input_files:
                raise Exception("Video.Matting.FileError")
            provider = data["provider"]
            if provider is None:
                raise Exception(f"Video.Matting.ParamError: provider: {provider}")
            mode = data["mode"]
            if mode is None:
                raise Exception(f"Video.Matting.ParamError: mode: {mode}")
            yield from self.base_service.start(input, output, input_files, provider, mode)
            self.base_service.is_stop = True
            self.base_service.message = {"type": "success", "text": "Video.Matting.ProcessEnd"}
        except ActivationException:
            self.base_service.is_auth = False
            self.base_service.message = {"type": "error", "text": "Video.Matting.ProcessAuthError"}
