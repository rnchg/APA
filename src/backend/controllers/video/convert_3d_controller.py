...
from core.services.pages.video.convert_3d_service import Convert3dService
...
class Convert3dController(BaseController):
    ...
    def setup_routes(self):
        ...
        def video_convert_3d_getFileUrl():
    ...
    def start(self, data):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception("Video.Convert3d.InputError")
            output = data["output"]
            if not os.path.isdir(output):
                raise Exception("Video.Convert3d.OutputError")
            input_files = data["input_files"]
            if not input_files:
                raise Exception("Video.Convert3d.FileError")
            provider = data["provider"]
            if provider is None:
                raise Exception(f"Video.Convert3d.ParamError: provider: {provider}")
            mode = data["mode"]
            if mode is None:
                raise Exception(f"Video.Convert3d.ParamError: mode: {mode}")
            format = data["format"]
            if format is None:
                raise Exception(f"Video.Convert3d.ParamError: format: {format}")
            shift = data["shift"]
            if shift is None:
                raise Exception(f"Video.Convert3d.ParamError: shift: {shift}")
            pop_out = data["pop_out"]
            if pop_out is None:
                raise Exception(f"Video.Convert3d.ParamError: pop_out: {pop_out}")
            cross_eye = data["cross_eye"]
            if cross_eye is None:
                raise Exception(f"Video.Convert3d.ParamError: cross_eye: {cross_eye}")
            yield from self.base_service.start(input, output, input_files, provider, mode, format, shift, pop_out, cross_eye)
            self.base_service.is_stop = True
            self.base_service.message = {"type": "success", "text": "Video.Convert3d.ProcessEnd"}
        except ActivationException:
            self.base_service.is_auth = False
            self.base_service.message = {"type": "error", "text": "Video.Convert3d.ProcessAuthError"}
