...
from core.services.image.auto_wipe.index_service import IndexService as AutoWipeService
    ...
    def start(self, data):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception("Image.AutoWipe.InputError")
            output = data["output"]
            if not os.path.isdir(output):
                raise Exception("Image.AutoWipe.OutputError")
            input_files = data["input_files"]
            if not input_files:
                raise Exception("Image.AutoWipe.FileError")
            provider = data["provider"]
            if provider is None:
                raise Exception(f"Image.AutoWipe.ParamError: provider: {provider}")
            mode = data["mode"]
            if mode is None:
                raise Exception(f"Image.AutoWipe.ParamError: mode: {mode}")
            mask = data["mask"]
            if mask is None:
                raise Exception(f"Image.AutoWipe.ParamError: mask: {mask}")
            ...
            self.base_service.is_stop = True
            self.base_service.message = {"type": "success", "text": "Image.AutoWipe.ProcessEnd"}
        except ActivationException:
            self.base_service.is_auth = False
            self.base_service.message = {"type": "error", "text": "Image.AutoWipe.ProcessAuthError"}
