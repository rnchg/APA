...
from core.services.pages.image.matting_service import MattingService
...
class MattingController(BaseController):
    def __init__(self):
        ...
        self.base_url = f"{self.base_api}/image/matting"
    ...
    def setup_routes(self):
        ...
        def image_matting_open():
            ...
            if not os.path.exists(path):
                return self.badRequest(message=f"Image.Matting.OpenError: path: {path}")
            os.startfile(path)
            return self.ok()
    ...
    def start(self, data):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception("Image.Matting.InputError")
            output = data["output"]
            if not os.path.isdir(output):
                raise Exception("Image.Matting.OutputError")
            input_files = data["input_files"]
            if not input_files:
                raise Exception("Image.Matting.FileError")
            provider = data["provider"]
            if provider is None:
                raise Exception(f"Image.Matting.ParamError: provider: {provider}")
            mode = data["mode"]
            if mode is None:
                raise Exception(f"Image.Matting.ParamError: mode: {mode}")
            yield from self.base_service.start(input, output, input_files, provider, mode)
            self.base_service.is_stop = True
            self.base_service.message = {"type": "success", "text": "Image.Matting.ProcessEnd"}
        except ActivationException:
            self.base_service.is_auth = False
            self.base_service.message = {"type": "error", "text": "Image.Matting.ProcessAuthError"}
