...
from core.services.pages.image.face_restoration_service import FaceRestorationService
...
class FaceRestorationController(BaseController):
    def __init__(self):
        ...
        self.base_service = FaceRestorationService()
        self.base_url = f"{self.base_api}/image/faceRestoration"

    def setup_routes(self):

        @self.base_server.route(f"{self.base_url}/getConfig", methods=["POST"])
        def image_face_restoration_getConfig():
            data = Current.config.image_face_restoration.to_dict()
            return self.ok(data)

        @self.base_server.route(f"{self.base_url}/setConfig", methods=["POST"])
        def image_face_restoration_setConfig():
            data = request.json["data"]
            Current.config.image_face_restoration.dict_to(data)
            return self.ok()

        @self.base_server.route(f"{self.base_url}/getFolder", methods=["POST"])
        def image_face_restoration_getFolder():
            path = request.json["path"]
            folders = webview.windows[0].create_file_dialog(webview.FOLDER_DIALOG, directory=path)
            if not folders:
                return self.badRequest(message=f"Image.FaceRestoration.FolderError: path: {path}")
            return self.ok({"folder": folders[0]})

        @self.base_server.route(f"{self.base_url}/getFileGrid", methods=["POST"])
        def image_face_restoration_getFileGrid():
        ...
        def image_face_restoration_getFileUrl():
    ...
    def start(self, data):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception("Image.FaceRestoration.InputError")
            output = data["output"]
            if not os.path.isdir(output):
                raise Exception("Image.FaceRestoration.OutputError")
            input_files = data["input_files"]
            if not input_files:
                raise Exception("Image.FaceRestoration.FileError")
            provider = data["provider"]
            if provider is None:
                raise Exception(f"Image.FaceRestoration.ParamError: provider: {provider}")
            mode = data["mode"]
            if mode is None:
                raise Exception(f"Image.FaceRestoration.ParamError: mode: {mode}")
            yield from self.base_service.start(input, output, input_files, provider, mode)
            self.base_service.is_stop = True
            self.base_service.message = {"type": "success", "text": "Image.FaceRestoration.ProcessEnd"}
        except ActivationException:
            self.base_service.is_auth = False
            self.base_service.message = {"type": "error", "text": "Image.FaceRestoration.ProcessAuthError"}
