...
from core.services.pages.image.cartoon_comic_service import CartoonComicService
...
class CartoonComicController(BaseController):
    def __init__(self):
        ...
        self.base_url = f"{self.base_api}/image/cartoonComic"

    def setup_routes(self):

        @server.route(f"{self.base_url}/getConfig", methods=["POST"])
        def image_cartoon_comic_getConfig():
            data = Current.config.image_cartoon_comic.to_dict()
            return self.ok(data)

        @server.route(f"{self.base_url}/setConfig", methods=["POST"])
        def image_cartoon_comic_setConfig():
            data = request.json["data"]
            Current.config.image_cartoon_comic.dict_to(data)
            return self.ok()
        
        @server.route(f"{self.base_url}/getFolder", methods=["POST"])
        def image_cartoon_comic_getFolder():
            path = request.json["path"]
            folders = webview.windows[0].create_file_dialog(webview.FOLDER_DIALOG, directory=path)
            if not folders:
                return self.badRequest(message=f"Image.CartoonComic.FolderError: path: {path}")
            return self.ok({"folder": folders[0]})

        @server.route(f"{self.base_url}/getFileGrid", methods=["POST"])
        def image_cartoon_comic_getFileGrid():
        ...
        def image_cartoon_comic_getFileUrl():
    ...
    def start(self, data):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception("Image.CartoonComic.InputError")
            output = data["output"]
            if not os.path.isdir(output):
                raise Exception("Image.CartoonComic.OutputError")
            input_files = data["input_files"]
            if not input_files:
                raise Exception("Image.CartoonComic.FileError")
            provider = data["provider"]
            if provider is None:
                raise Exception(f"Image.CartoonComic.ParamError: provider: {provider}")
            mode = data["mode"]
            if mode is None:
                raise Exception(f"Image.CartoonComic.ParamError: mode: {mode}")
            quality = data["quality"]
            if quality is None:
                raise Exception(f"Image.CartoonComic.ParamError: quality: {quality}")
            yield from self.base_service.start(input, output, input_files, provider, mode, quality)
            self.base_service.is_stop = True
            self.base_service.message = {"type": "success", "text": "Image.CartoonComic.ProcessEnd"}
        except ActivationException:
            self.base_service.is_auth = False
            self.base_service.message = {"type": "error", "text": "Image.CartoonComic.ProcessAuthError"}
