...
from core.services.pages.image.auto_wipe.index_service import IndexService
...
class IndexPage(BasePage):
    ...
    def init_language(self):
        self.input_label_var.set(Language.get("ImageAutoWipeIndexPageInputFolder"))
        self.input_button_var.set(Language.get("ImageAutoWipeIndexPageInputSelect"))
        self.output_label_var.set(Language.get("ImageAutoWipeIndexPageOutputFolder"))
        self.output_button_var.set(Language.get("ImageAutoWipeIndexPageOutputSelect"))
        self.provider_label_var.set(Language.get("ImageAutoWipeIndexPageProvider"))
        self.mode_label_var.set(Language.get("ImageAutoWipeIndexPageMode"))
        self.progress_label_var.set(Language.get("ImageAutoWipeIndexPageProgress"))
        self.start_button_var.set(Language.get("ImageAutoWipeIndexPageStart"))
        self.stop_button_var.set(Language.get("ImageAutoWipeIndexPageStop"))

        self.provider_source = Adapter.providers
        self.provider_combobox.configure(values=Option.get_text_list(self.provider_source))

        self.mode_source = [
            Option(Language.get("ImageAutoWipeIndexPageModeStandard"), "standard"),
        ]
        self.mode_combobox.configure(values=Option.get_text_list(self.mode_source))

        self.file_switch_source = [
            Option(Language.get("ImageAutoWipeIndexPageInputFolder"), "input"),
            Option(Language.get("ImageAutoWipeIndexPageOutputFolder"), "output"),
        ]
        ...
        self.add_message(Language.get("ImageAutoWipeHelp"), "success")
    ...
    def start(self):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception(Language.get("ImageAutoWipeIndexPageInputError"))
            output = self.output_entry_var.get()
            if not os.path.isdir(output):
                raise Exception(Language.get("ImageAutoWipeIndexPageOutputError"))
            input_files = self.file_grid.get_file_path()
            if input_files is None:
                raise Exception(Language.get("ImageAutoWipeIndexPageFileError"))
            provider = self.get_provider()
            if provider is None:
                raise Exception(f"{Language.get("ImageAutoWipeIndexPageParamError")}: provider: {provider}")
            mode = self.get_mode()
            if mode is None:
                raise Exception(f"{Language.get("ImageAutoWipeIndexPageParamError")}: mode: {mode}")
            mask = self.file_view.get_mask()
            if mask is None:
                raise Exception(f"{Language.get("ImageAutoWipeIndexPageParamError")}: mask: {mask}")
