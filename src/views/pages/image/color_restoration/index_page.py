...
from core.services.pages.image.color_restoration.index_service import IndexService
...
class IndexPage(BasePage):
    ...
    def init_language(self):
        self.input_label_var.set(Language.get("ImageColorRestorationIndexPageInputFolder"))
        self.input_button_var.set(Language.get("ImageColorRestorationIndexPageInputSelect"))
        self.output_label_var.set(Language.get("ImageColorRestorationIndexPageOutputFolder"))
        self.output_button_var.set(Language.get("ImageColorRestorationIndexPageOutputSelect"))
        self.provider_label_var.set(Language.get("ImageColorRestorationIndexPageProvider"))
        self.mode_label_var.set(Language.get("ImageColorRestorationIndexPageMode"))
        self.quality_label_var.set(Language.get("ImageColorRestorationIndexPageQuality"))
        self.progress_label_var.set(Language.get("ImageColorRestorationIndexPageProgress"))
        self.start_button_var.set(Language.get("ImageColorRestorationIndexPageStart"))
        self.stop_button_var.set(Language.get("ImageColorRestorationIndexPageStop"))

        self.provider_source = Adapter.providers
        self.provider_combobox.configure(values=Option.get_text_list(self.provider_source))

        self.mode_source = [
            Option(Language.get("ImageColorRestorationIndexPageModeStandard"), "standard"),
        ]
        self.mode_combobox.configure(values=Option.get_text_list(self.mode_source))

        self.quality_source = [
            Option(Language.get("ImageColorRestorationIndexPageQualityHigh"), "high"),
            Option(Language.get("ImageColorRestorationIndexPageQualityMedium"), "medium"),
            Option(Language.get("ImageColorRestorationIndexPageQualityLow"), "low"),
        ]
        self.quality_combobox.configure(values=Option.get_text_list(self.quality_source))

        self.file_switch_source = [
            Option(Language.get("ImageColorRestorationIndexPageInputFolder"), "input"),
            Option(Language.get("ImageColorRestorationIndexPageOutputFolder"), "output"),
        ]
        ...
        self.add_message(Language.get("ImageColorRestorationHelp"), "success")
    ...
    def start(self):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception(Language.get("ImageColorRestorationIndexPageInputError"))
            output = self.output_entry_var.get()
            if not os.path.isdir(output):
                raise Exception(Language.get("ImageColorRestorationIndexPageOutputError"))
            input_files = self.file_grid.get_file_path()
            if not input_files:
                raise Exception(Language.get("ImageColorRestorationIndexPageFileError"))
            provider = self.get_provider()
            if provider is None:
                raise Exception(f"{Language.get("ImageColorRestorationIndexPageParamError")}: provider: {provider}")
            mode = self.get_mode()
            if mode is None:
                raise Exception(f"{Language.get("ImageColorRestorationIndexPageParamError")}: mode: {mode}")
            quality = self.get_quality()
            if quality is None:
                raise Exception(f"{Language.get("ImageColorRestorationIndexPageParamError")}: quality: {quality}")
