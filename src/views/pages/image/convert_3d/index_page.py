...
from core.services.pages.image.convert_3d.index_service import IndexService
...
class IndexPage(BasePage):
    ...
    def init_language(self):
        self.input_label_var.set(Language.get("ImageConvert3dIndexPageInputFolder"))
        self.input_button_var.set(Language.get("ImageConvert3dIndexPageInputSelect"))
        self.output_label_var.set(Language.get("ImageConvert3dIndexPageOutputFolder"))
        self.output_button_var.set(Language.get("ImageConvert3dIndexPageOutputSelect"))
        self.provider_label_var.set(Language.get("ImageConvert3dIndexPageProvider"))
        self.mode_label_var.set(Language.get("ImageConvert3dIndexPageMode"))
        self.format_label_var.set(Language.get("ImageConvert3dIndexPageFormat"))
        self.shift_label_var.set(Language.get("ImageConvert3dIndexPageShift"))
        self.pop_out_label_var.set(Language.get("ImageConvert3dIndexPagePopOut"))
        self.cross_eye_label_var.set(Language.get("ImageConvert3dIndexPageCrossEye"))
        self.progress_label_var.set(Language.get("ImageConvert3dIndexPageProgress"))
        self.start_button_var.set(Language.get("ImageConvert3dIndexPageStart"))
        self.stop_button_var.set(Language.get("ImageConvert3dIndexPageStop"))

        self.provider_source = Adapter.providers
        self.provider_combobox.configure(values=Option.get_text_list(self.provider_source))

        self.mode_source = [
            Option(Language.get("ImageConvert3dIndexPageModeStandard"), "standard"),
        ]
        self.mode_combobox.configure(values=Option.get_text_list(self.mode_source))

        self.format_source = [
            Option(Language.get("ImageConvert3dIndexPageFormatHalfSbs"), "half_sbs"),
            Option(Language.get("ImageConvert3dIndexPageFormatSbs"), "sbs"),
            Option(Language.get("ImageConvert3dIndexPageFormatAnaglyph"), "anaglyph"),
            Option(Language.get("ImageConvert3dIndexPageFormatDepth"), "depth"),
        ]
        self.format_combobox.configure(values=Option.get_text_list(self.format_source))

        self.shift_source = [
            Option(Language.get("ImageConvert3dIndexPageShift10"), 10),
            Option(Language.get("ImageConvert3dIndexPageShift20"), 20),
            Option(Language.get("ImageConvert3dIndexPageShift30"), 30),
            Option(Language.get("ImageConvert3dIndexPageShift50"), 50),
            Option(Language.get("ImageConvert3dIndexPageShift100"), 100),
            Option(Language.get("ImageConvert3dIndexPageShift200"), 200),
            Option(Language.get("ImageConvert3dIndexPageShift300"), 300),
            Option(Language.get("ImageConvert3dIndexPageShift500"), 500),
            Option(Language.get("ImageConvert3dIndexPageShift1000"), 1000),
        ]
        self.shift_combobox.configure(values=Option.get_text_list(self.shift_source))

        self.file_switch_source = [
            Option(Language.get("ImageConvert3dIndexPageInputFolder"), "input"),
            Option(Language.get("ImageConvert3dIndexPageOutputFolder"), "output"),
        ]
        ...
        self.add_message(Language.get("ImageConvert3dHelp"), "success")
    ...
    def start(self):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception(Language.get("ImageConvert3dIndexPageInputError"))
            output = self.output_entry_var.get()
            if not os.path.isdir(output):
                raise Exception(Language.get("ImageConvert3dIndexPageOutputError"))
            input_files = self.file_grid.get_file_path()
            if not input_files:
                raise Exception(Language.get("ImageConvert3dIndexPageFileError"))
            provider = self.get_provider()
            if provider is None:
                raise Exception(f"{Language.get("ImageConvert3dIndexPageParamError")}: provider: {provider}")
            mode = self.get_mode()
            if mode is None:
                raise Exception(f"{Language.get("ImageConvert3dIndexPageParamError")}: mode: {mode}")
            format = self.get_format()
            if format is None:
                raise Exception(f"{Language.get("ImageConvert3dIndexPageParamError")}: format: {format}")
            shift = self.get_shift()
            if shift is None:
                raise Exception(f"{Language.get("ImageConvert3dIndexPageParamError")}: shift: {shift}")
            pop_out = self.get_pop_out()
            if pop_out is None:
                raise Exception(f"{Language.get("ImageConvert3dIndexPageParamError")}: pop_out: {pop_out}")
            cross_eye = self.get_cross_eye()
            if cross_eye is None:
                raise Exception(f"{Language.get("ImageConvert3dIndexPageParamError")}: cross_eye: {cross_eye}")
