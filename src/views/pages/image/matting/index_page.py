...
from core.services.pages.image.matting.index_service import IndexService
...
class IndexPage(BasePage):
    ...
    def init_language(self):
        self.input_label_var.set(Language.get("ImageMattingIndexPageInputFolder"))
        self.input_button_var.set(Language.get("ImageMattingIndexPageInputSelect"))
        self.output_label_var.set(Language.get("ImageMattingIndexPageOutputFolder"))
        self.output_button_var.set(Language.get("ImageMattingIndexPageOutputSelect"))
        self.provider_label_var.set(Language.get("ImageMattingIndexPageProvider"))
        self.mode_label_var.set(Language.get("ImageMattingIndexPageMode"))
        self.progress_label_var.set(Language.get("ImageMattingIndexPageProgress"))
        self.start_button_var.set(Language.get("ImageMattingIndexPageStart"))
        self.stop_button_var.set(Language.get("ImageMattingIndexPageStop"))

        self.provider_source = Adapter.providers
        self.provider_combobox.configure(values=Option.get_text_list(self.provider_source))

        self.mode_source = [
            Option(Language.get("ImageMattingIndexPageModeStandard"), "standard"),
        ]
        self.mode_combobox.configure(values=Option.get_text_list(self.mode_source))

        self.file_switch_source = [
            Option(Language.get("ImageMattingIndexPageInputFolder"), "input"),
            Option(Language.get("ImageMattingIndexPageOutputFolder"), "output"),
        ]
        ...
        self.add_message(Language.get("ImageMattingHelp"), "success")
    ...
    def start(self):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception(Language.get("ImageMattingIndexPageInputError"))
            output = self.output_entry_var.get()
            if not os.path.isdir(output):
                raise Exception(Language.get("ImageMattingIndexPageOutputError"))
            input_files = self.file_grid.get_file_path()
            if not input_files:
                raise Exception(Language.get("ImageMattingIndexPageFileError"))
            provider = self.get_provider()
            if provider is None:
                raise Exception(f"{Language.get("ImageMattingIndexPageParamError")}: provider: {provider}")
            mode = self.get_mode()
            if mode is None:
                raise Exception(f"{Language.get("ImageMattingIndexPageParamError")}: mode: {mode}")
