...
from core.services.pages.video.matting.index_service import IndexService
...
class IndexPage(BasePage):
    ...
    def init_language(self):
        self.input_label_var.set(Language.get("VideoMattingIndexPageInputFolder"))
        self.input_button_var.set(Language.get("VideoMattingIndexPageInputSelect"))
        self.output_label_var.set(Language.get("VideoMattingIndexPageOutputFolder"))
        self.output_button_var.set(Language.get("VideoMattingIndexPageOutputSelect"))
        self.provider_label_var.set(Language.get("VideoMattingIndexPageProvider"))
        self.mode_label_var.set(Language.get("VideoMattingIndexPageMode"))
        self.progress_label_var.set(Language.get("VideoMattingIndexPageProgress"))
        self.start_button_var.set(Language.get("VideoMattingIndexPageStart"))
        self.stop_button_var.set(Language.get("VideoMattingIndexPageStop"))

        self.provider_source = Adapter.providers
        self.provider_combobox.configure(values=Option.get_text_list(self.provider_source))

        self.mode_source = [
            Option(Language.get("VideoMattingIndexPageModeStandard"), "standard"),
        ]
        self.mode_combobox.configure(values=Option.get_text_list(self.mode_source))

        self.file_switch_source = [
            Option(Language.get("VideoMattingIndexPageInputFolder"), "input"),
            Option(Language.get("VideoMattingIndexPageOutputFolder"), "output"),
        ]
        ...
        self.add_message(Language.get("VideoMattingHelp"), "success")
    ...
    def start(self):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception(Language.get("VideoMattingIndexPageInputError"))
            output = self.output_entry_var.get()
            if not os.path.isdir(output):
                raise Exception(Language.get("VideoMattingIndexPageOutputError"))
            input_files = self.file_grid.get_file_path()
            if not input_files:
                raise Exception(Language.get("VideoMattingIndexPageFileError"))
            provider = self.get_provider()
            if provider is None:
                raise Exception(f"{Language.get("VideoMattingIndexPageParamError")}: provider: {provider}")
            mode = self.get_mode()
            if mode is None:
                raise Exception(f"{Language.get("VideoMattingIndexPageParamError")}: mode: {mode}")
