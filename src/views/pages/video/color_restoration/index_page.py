...
from core.services.pages.video.color_restoration.index_service import IndexService
...
class IndexPage(BasePage):
    ...
    def init_language(self):
        self.input_label_var.set(Language.get("VideoColorRestorationIndexPageInputFolder"))
        self.input_button_var.set(Language.get("VideoColorRestorationIndexPageInputSelect"))
        self.output_label_var.set(Language.get("VideoColorRestorationIndexPageOutputFolder"))
        self.output_button_var.set(Language.get("VideoColorRestorationIndexPageOutputSelect"))
        self.provider_label_var.set(Language.get("VideoColorRestorationIndexPageProvider"))
        self.mode_label_var.set(Language.get("VideoColorRestorationIndexPageMode"))
        self.quality_label_var.set(Language.get("VideoColorRestorationIndexPageQuality"))
        self.progress_label_var.set(Language.get("VideoColorRestorationIndexPageProgress"))
        self.start_button_var.set(Language.get("VideoColorRestorationIndexPageStart"))
        self.stop_button_var.set(Language.get("VideoColorRestorationIndexPageStop"))

        self.provider_source = Adapter.providers
        self.provider_combobox.configure(values=Option.get_text_list(self.provider_source))

        self.mode_source = [
            Option(Language.get("VideoColorRestorationIndexPageModeStandard"), "standard"),
        ]
        self.mode_combobox.configure(values=Option.get_text_list(self.mode_source))

        self.quality_source = [
            Option(Language.get("VideoColorRestorationIndexPageQualityHigh"), "high"),
            Option(Language.get("VideoColorRestorationIndexPageQualityMedium"), "medium"),
            Option(Language.get("VideoColorRestorationIndexPageQualityLow"), "low"),
        ]
        self.quality_combobox.configure(values=Option.get_text_list(self.quality_source))

        self.file_switch_source = [
            Option(Language.get("VideoColorRestorationIndexPageInputFolder"), "input"),
            Option(Language.get("VideoColorRestorationIndexPageOutputFolder"), "output"),
        ]
        ...
        self.add_message(Language.get("VideoColorRestorationHelp"), "success")
    ...
    def start(self):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception(Language.get("VideoColorRestorationIndexPageInputError"))
            output = self.output_entry_var.get()
            if not os.path.isdir(output):
                raise Exception(Language.get("VideoColorRestorationIndexPageOutputError"))
            input_files = self.file_grid.get_file_path()
            if not input_files:
                raise Exception(Language.get("VideoColorRestorationIndexPageFileError"))
            provider = self.get_provider()
            if provider is None:
                raise Exception(f"{Language.get("VideoColorRestorationIndexPageParamError")}: provider: {provider}")
            mode = self.get_mode()
            if mode is None:
                raise Exception(f"{Language.get("VideoColorRestorationIndexPageParamError")}: mode: {mode}")
            quality = self.get_quality()
            if quality is None:
                raise Exception(f"{Language.get("VideoColorRestorationIndexPageParamError")}: quality: {quality}")
