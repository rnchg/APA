...
from core.services.pages.audio.denoise.index_service import IndexService
...
class IndexPage(BasePage):
    ...
    def init_language(self):
        self.input_label_var.set(Language.get("AudioDenoiseIndexPageInputFolder"))
        self.input_button_var.set(Language.get("AudioDenoiseIndexPageInputSelect"))
        self.output_label_var.set(Language.get("AudioDenoiseIndexPageOutputFolder"))
        self.output_button_var.set(Language.get("AudioDenoiseIndexPageOutputSelect"))
        self.provider_label_var.set(Language.get("AudioDenoiseIndexPageProvider"))
        self.mode_label_var.set(Language.get("AudioDenoiseIndexPageMode"))
        self.progress_label_var.set(Language.get("AudioDenoiseIndexPageProgress"))
        self.start_button_var.set(Language.get("AudioDenoiseIndexPageStart"))
        self.stop_button_var.set(Language.get("AudioDenoiseIndexPageStop"))

        self.provider_source = Adapter.providers
        self.provider_combobox.configure(values=Option.get_text_list(self.provider_source))

        self.mode_source = [
            Option(Language.get("AudioDenoiseIndexPageModeStandard"), "standard"),
        ]
        self.mode_combobox.configure(values=Option.get_text_list(self.mode_source))

        self.file_switch_source = [
            Option(Language.get("AudioDenoiseIndexPageInputFolder"), "input"),
            Option(Language.get("AudioDenoiseIndexPageOutputFolder"), "output"),
        ]
        ...
        self.add_message(Language.get("AudioDenoiseHelp"), "success")
    ...
    def start(self):
        try:
            ...
            if not os.path.isdir(input):
                raise Exception(Language.get("AudioDenoiseIndexPageInputError"))
            output = self.output_entry_var.get()
            if not os.path.isdir(output):
                raise Exception(Language.get("AudioDenoiseIndexPageOutputError"))
            input_files = self.file_grid.get_file_path()
            if not input_files:
                raise Exception(Language.get("AudioDenoiseIndexPageFileError"))
            provider = self.get_provider()
            if provider is None:
                raise Exception(f"{Language.get("AudioDenoiseIndexPageParamError")}: provider: {provider}")
            mode = self.get_mode()
            if mode is None:
                raise Exception(f"{Language.get("AudioDenoiseIndexPageParamError")}: mode: {mode}")
